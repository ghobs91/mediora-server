...
export class SetupStateService {
  ...
  public async getStatus(): Promise<SetupStatus> {
    const [completed, tmdbApiKey, jackettApiKey, passwordHash] =
      await Promise.all([
        this.parameterDAO.findOne({ key: ParameterKey.SETUP_COMPLETED }),
        this.parameterDAO.findOne({ key: ParameterKey.TMDB_API_KEY }),
        this.parameterDAO.findOne({ key: ParameterKey.JACKETT_API_KEY }),
        this.parameterDAO.findOne({ key: ParameterKey.AUTH_PASSWORD_HASH }),
      ]);
